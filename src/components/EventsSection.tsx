      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          ...
          {[1, 2, 3, 4, 5, 6].map((index) => (
            <div 
              key={index}
              className={`content-card group relative aspect-square transition-all duration-1000 delay-${index * 100} ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
            >
              <div className="absolute inset-0 bg-gradient-to-br from-shayo-purple/40 to-shayo-pink/40 rounded-xl overflow-hidden"></div>
              
              <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black to-transparent">
                <div className="flex justify-between items-center">
                  <span className="text-white font-bold">Event #{index}</span>
